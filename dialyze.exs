defmodule Dialyze do
  ...
  defp filter_warning(warning = {:warn_matching, {'lib/kernel.ex', line}, {:pattern_match, ['pattern \'false\'', '\'true\'']}}) when line in [1842, 3153, 3506, 3815, 4161, 4165, 4624],
    do: {:ok, "inlined bootstrap check stuff", warning, 3}
  expected = Map.put(expected, 3, 1)
  ...
  expected = Map.put(expected, 15, 1)
  ...
  expected = Map.put(expected, 16, 1)
  ...
  expected = Map.put(expected, 17, 1)
  ...
  expected = Map.put(expected, 18, 1)
  ...
  expected = Map.put(expected, 19, 1)
  ...
  expected = Map.put(expected, 20, 1)
  ...
  expected = Map.put(expected, 21, 1)
...
#   |> Enum.reduce(%{}, fn
#     {:warning, _}, acc -> acc
#     {:ok, _, _, id}, acc -> Map.update(acc, id, 1, & &1 + 1)
#   end)
# IO.inspect(counts, label: "found")
# IO.inspect(Dialyze.expected_counts(), label: "expected")
