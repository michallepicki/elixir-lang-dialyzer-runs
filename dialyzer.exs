defmodule Dialyzer do
  ...
  defp filter(
         dialyzer_warning =
           {:warn_matching, {~c"lib/dynamic_supervisor.ex", {477, 8}},
            {:pattern_match_cov,
             [
               ~c"variable _other@1",
  ...
  defp filter(
         dialyzer_warning =
            {:warn_matching, {~c"lib/iex/info.ex", {_, 7}}, {:pattern_match_cov, [~c"variable _doc@2", ~c"{{'doc_cons','doc_line' | 'doc_nil' | binary() | tuple(),'doc_line' | 'doc_nil' | binary() | tuple()} | {'doc_group','doc_line' | 'doc_nil' | binary() | {_,_} | {_,_,_} | {_,_,_,_},'inherit' | 'normal' | 'optimistic' | 'pessimistic'},\#{'__struct__':='Elixir.Inspect.Opts', 'base':='binary' | 'decimal' | 'hex' | 'octal', 'binaries':='as_binaries' | 'as_strings' | 'infer', 'char_lists':=_, 'charlists':='as_charlists' | 'as_lists' | 'infer', 'custom_options':=[any()], 'inspect_fun':=fun((_,_) -> any()), 'limit':='infinity' | non_neg_integer(), 'pretty':=boolean(), 'printable_limit':='infinity' | non_neg_integer(), 'safe':=boolean(), 'structs':=boolean(), 'syntax_colors':=[any()], 'width':='infinity' | non_neg_integer()}}"]}}
       ),
       do: filtered(comment: "overly defensive code", id: @id, data: dialyzer_warning)
