defmodule Dialyzer do
  ...
  defp filter(
         dialyzer_warning =
           {:warn_matching, {~c"lib/calendar/date_range.ex", {218, 10}},
            {:pattern_match_cov,
             [
  ...
  defp filter(
         dialyzer_warning =
           {:warn_callgraph, {~c"src/elixir_erl_compiler.erl", {68, 24}}, {:call_to_missing, [:sys_coverage, :beam_debug_info, 1]}}
       ),
       do: filtered(comment: "function effectively called only on otp 28.0+, when the beam_debug_info compiler option was added", id: @id, data: dialyzer_warning)
  ...
  defp filter(
         dialyzer_warning =
           {:warn_matching, {~c"lib/module/types/descr.ex", {_, 7}}, {:pattern_match_cov, [~c"variable _", ~c"'all_equal' | 'left_subtype_of_right' | 'right_subtype_of_left' | {'one_key_difference',_,_,_}"]}}
       ),
       do: filtered(comment: "overly defensive code", id: @id, data: dialyzer_warning)
  ...
  defp filter(
         dialyzer_warning =
           {:warn_return_no_exit, {~c"lib/string.ex", {1131, 7}}, {:no_return, [:only_normal, :__to_existing_atom__, 2]}}
       ),
       do: filtered(comment: "not annotated exception", id: @id, data: dialyzer_warning)
  ...
  defp filter(
         dialyzer_warning =
           {:warn_return_no_exit, {~c"lib/list.ex", {3111, 7}}, {:no_return, [:only_normal, :__to_existing_atom__, 2]}}
       ),
       do: filtered(comment: "not annotated exception", id: @id, data: dialyzer_warning)
  ...
  defp filter(
         dialyzer_warning =
           {:warn_return_no_exit, {~c"lib/mix/tasks/deps.partition.ex", {256, 11}}, {:no_return, [:only_normal]}}
       ),
       do: filtered(comment: "not annotated exit", id: @id, data: dialyzer_warning)
  ...
  defp filter(
         dialyzer_warning =
           {:warn_return_no_exit, {~c"lib/mix/tasks/deps.partition.ex", {192, 8}}, {:no_return, [:only_normal, :tcp_failed!, 4]}}
       ),
       do: filtered(comment: "not annotated exit", id: @id, data: dialyzer_warning)
